...
#Region " Windows Form Designer generated code "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        ...
        'DataGridTableStyle1
        '
        Me.DataGridTableStyle1.DataGrid = Me.dgCommRun
        ...
        Me.bStatus.Location = New System.Drawing.Point(8, 40)
        ...
        Me.cr1.Location = New System.Drawing.Point(592, 128)
        Me.cr1.Name = "cr1"
        Me.cr1.ReportSource = Nothing
...
#End Region
    ...
    Dim BaseLocation = "\\storage\ToBePublished\MF\"
    Dim BatchStatus As Integer

    Private Sub CommRun(ByVal Opt)
        ''TC1.Visible = False
        ...
        sqlCmd.CommandText = "[WinfundCommProcess]"
    ...
    Private Sub BalApprove_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BalApprove.Click
        ...
        Try
            If BatchStatus = 0 Then '' Batch from new to Balanced
                If MsgBox("Are you the Numbers are matching with winfund?", MsgBoxStyle.YesNo) = MsgBoxResult.Yes Then
                    CommRun(6)
                    ProcessDate.Enabled = False
                Else
                    Exit Sub
            ...
            If BatchStatus = 1 Then '' Batch from  Balanced to Approved
                ...
                If MsgBox("Please double check before approving that numbers are matching with winfund.", MsgBoxStyle.YesNo) = MsgBoxResult.Yes Then
                    CommRun(7)
    ...
    Private Sub GenrateReports()
        ...
        sqlCmd.CommandText = "select distinct  tmpcommrunmf.rep_code,tmpcommrunmf.dealer_code,email_address = case when dealer_code in ( '7625','3346') then email else '' end ,RepNo =tmpcommrunmf.rep_code  from tmpcommrunmf , ibm.[global db].dbo.SalesInfo s where tmpcommrunmf.rep_code=s.repno and  isnumeric(tmpcommrunmf.rep_code) = 1 and  rep_code not in ( select code  from   ibm.[global db].dbo.codesMF where isPIStransferHist = 1 )  " _
        & " union select distinct  tmpcommrunmf.rep_code,tmpcommrunmf.dealer_code,email_address =   case when dealer_code in ( '7625','3346') then email else '' end,RepNo =tmpcommrunmf.rep_code  from tmpcommrunmf , ibm.[global db].dbo.SalesInfo s where   tmpcommrunmf.rep_code=s.repno and  Dealer_Code in ( '7686', '3365','7820' )  and isnumeric(tmpcommrunmf.rep_code) = 1 and   rep_code  in ( select code  from   ibm.[global db].dbo.codesMF where isPIStransferHist = 1   )  " _
        & " union  select distinct  tmpcommrunmf.rep_code,tmpcommrunmf.dealer_code,email_address  = case when tmpcommrunmf.dealer_code in ( '7625','3466') then email else '' end,RepNo =codesmf.repno from tmpcommrunmf, ibm.[global db].dbo.codesMF, ibm.[global db].dbo.SalesInfo s  where tmpcommrunmf.rep_code = codesmf.code and codesmf.repno=s.repno and   rep_code in ( select code  from   ibm.[global db].dbo.codesMF where isPIStransferHist = 1 ) and  tmpcommrunmf.Dealer_Code not in ( '7686', '3365','7820' )   and codesmf.repno > 0  " _
        & " union select distinct  tmpcommrunmf.rep_code,tmpcommrunmf.dealer_code,email_address  = case when tmpcommrunmf.dealer_code in ( '7625','3466') then email else '' end,RepNo =codesmf.repno from tmpcommrunmf, ibm.[global db].dbo.codesMF, ibm.[global db].dbo.SalesInfo s where tmpcommrunmf.rep_code = codesmf.code and codesmf.repno=s.repno and   isnumeric(tmpcommrunmf.rep_code) = 0 and rep_code  not in ( select code  from   ibm.[global db].dbo.codesMF where isPIStransferHist = 1 )  and codesmf.repno > 0      " _
        & " order by tmpcommrunmf.rep_code,tmpcommrunmf.dealer_code"


        '' dc on 2010-10-14   sqlCmd.CommandText = "select distinct  tmpcommrunmf.rep_code,tmpcommrunmf.dealer_code,email_address = 'kimberlyh@globalfinancial.ca',RepNo =tmpcommrunmf.rep_code from tmpcommrunmf where isnumeric(tmpcommrunmf.rep_code) = 1 union select distinct  tmpcommrunmf.rep_code,tmpcommrunmf.dealer_code,email_address = 'kimberlyh@globalfinancial.ca',RepNo =codesmf.repno from tmpcommrunmf, ibm.[global db].dbo.codesMF where tmpcommrunmf.rep_code = codesmf.code and isnumeric(tmpcommrunmf.rep_code) = 0  and repno > 0 order by tmpcommrunmf.rep_code,tmpcommrunmf.dealer_code"
        ...
        '' dc on 2010-10-14 for PIS mang emails   sqlCmd.CommandText = "select distinct  tmpcommrunmfmang.manager_code,tmpcommrunmfmang.dealer_code,email_address =  'kimberlyh@globalfinancial.ca',Repno = case when codesmf.repno = 0 then codesmf.Agencyno  else str(codesmf.repno) end from tmpcommrunmfMang,ibm.[global db].dbo.codesmf  where codesmf.code = tmpcommrunmfmang.manager_code  Union select distinct  tmpcommrunmfmang.manager_code,tmpcommrunmfmang.dealer_code,email_address =  '',Repno = tmpcommrunmfmang.manager_code from tmpcommrunmfMang where isnumeric(manager_code) = 1  order by tmpcommrunmfmang.manager_code,tmpcommrunmfmang.dealer_code"

        ''2010-10-14 added for PIS adv emails
        sqlCmd.CommandText = "select distinct  tmpcommrunmfmang.manager_code,tmpcommrunmfmang.dealer_code,email_address =  '',Repno = case when codesmf.repno = 0 then codesmf.Agencyno  else str(codesmf.repno) end from tmpcommrunmfMang,ibm.[global db].dbo.codesmf  where codesmf.code = tmpcommrunmfmang.manager_code " _
                                & " Union select distinct  tmpcommrunmfmang.manager_code,tmpcommrunmfmang.dealer_code,email_address =  email ,Repno = case when codesmf.repno = 0 then codesmf.Agencyno  else str(codesmf.repno) end from tmpcommrunmfMang,ibm.[global db].dbo.codesmf  , ibm.[global db].dbo.SalesInfo s  where tmpcommrunmfmang.dealer_code in ('7625','3466') and codesmf.code = tmpcommrunmfmang.manager_code  and codesmf.repno=s.repno" _
                                ...
                                & "union select distinct  tmpcommrunmfmang.manager_code,tmpcommrunmfmang.dealer_code,email_address =  email ,Repno = tmpcommrunmfmang.manager_code from tmpcommrunmfMang  , ibm.[global db].dbo.SalesInfo s where  tmpcommrunmfmang.dealer_code in ('7625','3466') and isnumeric(manager_code) = 1  and tmpcommrunmfMang.manager_code=s.repno"

        dr = sqlCmd.ExecuteReader

        While dr.Read()
            PdfReportMang(dr.Item("Dealer_code"), Trim(dr.Item("manager_code")), Microsoft.VisualBasic.Right("00000" & Trim(dr.Item("RepNo")), 5), "")  'dr("email_Address")
        End While
    ...
    Private Sub PdfReportRep(ByVal Dealer_code As String, ByVal rep_code As String, ByVal RepNO As String, ByVal Email As String)
        ...
        crConnectionInfo.DatabaseName = "WinFundSQL"
        crConnectionInfo.ServerName = "mfsrv"
        ...
        If Email.Length > 0 Then '' 201010-14 to send email to PIS Advisor Only
    ...
    Private Sub PdfReportMang(ByVal Dealer_code As String, ByVal manager_code As String, ByVal RepNO As String, ByVal email As String)
        ...
        If email.Length > 0 Then '' 201010-14 to send email to PIS Manager Only
